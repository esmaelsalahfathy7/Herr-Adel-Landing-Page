      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-20">
          ...
          <a href="#" className="flex items-center gap-3 group focus:outline-hidden">
            <div className="w-11 h-11 rounded-xl bg-slate-900 text-amber-400 flex items-center justify-center font-bold text-xl shadow-md group-hover:scale-105 transition-transform duration-300 border border-slate-800">
              AG
            </div>
            <div className="flex flex-col">
              <span className="font-extrabold text-lg sm:text-xl text-slate-900 tracking-tight group-hover:text-rose-600 transition-colors">
                {t.brandName}
              </span>
          ...
          <nav className="hidden md:flex items-center gap-8 font-medium text-sm text-slate-700">
            <a href="#about" className="hover:text-rose-600 transition-colors py-2">
              {t.about}
            </a>
            <a href="#courses" className="hover:text-rose-600 transition-colors py-2">
              {t.courses}
            </a>
            <a href="#testimonials" className="hover:text-rose-600 transition-colors py-2">
              {t.testimonials}
            </a>
            <a href="#faq" className="hover:text-rose-600 transition-colors py-2">
              {t.faq}
            </a>
            <a href="#contact" className="hover:text-rose-600 transition-colors py-2">
              {t.contact}
            </a>
          </nav>
          ...
          <div className="hidden md:flex items-center gap-4">
            ...
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2 px-3 py-2 rounded-lg bg-slate-100 hover:bg-slate-200/80 text-slate-800 font-semibold text-xs transition-all border border-slate-200 cursor-pointer"
              ...
              {langDropdownOpen && (
                <div className="absolute right-0 ltr:right-0 rtl:left-0 mt-2 w-36 bg-white rounded-xl shadow-xl border border-slate-200 py-1 z-50 animate-in fade-in zoom-in-95 duration-150">
                  {languages.map((lang) => (
                    <button
                      key={lang.code}
                      onClick={() => {
                      ...
                      className={`w-full flex items-center justify-between px-4 py-2 text-xs font-semibold hover:bg-slate-50 transition-colors ${
                        currentLang === lang.code ? 'text-rose-600 bg-rose-50/50' : 'text-slate-700'
                      }`}
                    >
                      ...
                      {currentLang === lang.code && <span className="w-1.5 h-1.5 rounded-full bg-rose-600"></span>}
                    </button>
                  ))}
                </div>
              )}
